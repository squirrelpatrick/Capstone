...
model12b <- glm(ResH ~ GDrdiff + GDrdiffLS, data = premiership3F15, family = "binomial")
model13b <- glm(ResH ~ GDrdiff, data = filter(premiership3F15, !is.na(GDrdiffLS)), family = "binomial")
model12b
model13b
...
premiership6$Team1Pscaled <- premiership6$Team1P / 38
premiership6$Team2Pscaled <- premiership6$Team2P / 38
...
model17 <- rpart(FTR ~ GDrdiff + GDrdiffLS + GDrdiffLS2 + Team1Pscaled + 
                Team2Pscaled, data = train, method = "class")
pred17 <- predict(model17, 
                  select(test, GDrdiff, GDrdiffLS, GDrdiffLS2, 
                         Team1Pscaled, Team2Pscaled), 
                  type = "prob")
...
model19 <- randomForest(FTR ~ GDrdiff + GDrdiffLS + GDrdiffLS2 + Team1Pscaled + 
                        Team2Pscaled, data = train, 
                        ntree = 2500,
                        type = "regression")
...
model20h <- glm(ResH ~ GDrdiff + GDrdiffLS + GDrdiffLS2 + Team1Pscaled + Team2Pscaled, data = train, family = "binomial")
model20a <- glm(ResA ~ GDrdiff + GDrdiffLS + GDrdiffLS2 + Team1Pscaled + Team2Pscaled, data = train, family = "binomial")
pred20h <- predict(model20h, select(test, GDrdiff, GDrdiffLS, 
                                  GDrdiffLS2, Team1Pscaled, Team2Pscaled),
                  type = "response")
...
model21 <- multinom(FTR ~ GDrdiff + GDrdiffLS + GDrdiffLS2 + Team1Pscaled + 
                      Team2Pscaled, train)
...
# playing around
train$newvar <- train$GDrdiff * sqrt(train$Team1Pscaled + train$Team2Pscaled)
test$newvar <- test$GDrdiff + sqrt(test$Team1Pscaled + test$Team2Pscaled)
...
model24 <- rpart(FTR ~ GDrdiff + GDrdiffLS + GDrdiffLS2 + Team1Pscaled + 
                              Team2Pscaled, minsplit = 50, cp = 0.003, data = train, method = "class")
...
model26 <- lm(Supr ~ GDrdiffLS + a:Team1Pscaled + a + b:Team1Pscaled + b, data = train)
summary(model26)

# very significant for a:Team1P :)
train <- train %>%
          mutate(c = 0.5* (Team1Pscaled + Team2Pscaled))
...
# x is the vector of the probability predicted for the actual outcome
likelihood <- function(x) {
  exp(sum(log(x))/length(x))
}
# also include accuracy function
accuracy <- function(x, y) {
  if(length(x) != length(y)) {
    print("vectors not equal length")
    return(0)
  }
  sum(x == y) / length(x)
}
# function to convert matrix of probabilities back to class
# precursor to calculating accuracy

matrix2class <- function(x) {
  c("H", "D", "A")[apply(x, 1, which.max)]
}
...
DecTreeLogRegProbMatrix <- matrix(0, nrow = nrow(premiership7), ncol = 3)
for (i in 1:10) {
  print(i)
  for (j in c(2,6,7)) {
    ...
    DecTreeLogRegProbMatrix[testlogi,1] <- predict(modelH, premiership7[testlogi,], type = "response")
    DecTreeLogRegProbMatrix[testlogi,3] <- predict(modelA, premiership7[testlogi,], type = "response")
    DecTreeLogRegProbMatrix[testlogi,2] <- 1 - DecTreeLogRegProbMatrix[testlogi,1] -  DecTreeLogRegProbMatrix[testlogi,3]
    }
}
DecTreeLogRegClassVector <- matrix2class(DecTreeLogRegProbMatrix)
DecTreeLogRegPredictions <- 
  DecTreeLogRegProbMatrix[,1] * (premiership7$FTR == "H") + 
  DecTreeLogRegProbMatrix[,2] * (premiership7$FTR == "D") +
  DecTreeLogRegProbMatrix[,3] * (premiership7$FTR == "A")


cat("Decision Tree + logistic Regression K-fold likelihood metric",
    likelihood(DecTreeLogRegPredictions)
)
cat("Decision Tree + logistic Regression K-fold accuracy",
    accuracy(DecTreeLogRegClassVector, premiership7$FTR)
)
...
DecTreeLogRegVProbMatrix <- matrix(0, nrow = nrow(premiership7V), ncol = 3)
colnames(DecTreeLogRegVProbMatrix) = c("H", "D", "A")
for (i in c(2,6,7)) {
  nodelogi <- premiership7V$DTnode == i
  modelH <- glm(ResH ~ GDrdiffLS + a:c + b, filter(premiership7, DTnode == i), family = "binomial")
  modelA <- glm(ResA ~ GDrdiffLS + a:c + b, filter(premiership7, DTnode == i), family = "binomial")
DecTreeLogRegVProbMatrix[nodelogi, 1] <- predict(modelH, premiership7V[nodelogi,], type = "response")
DecTreeLogRegVProbMatrix[nodelogi, 3] <- predict(modelA, premiership7V[nodelogi,], type = "response")
DecTreeLogRegVProbMatrix[nodelogi, 2] <- 1 - DecTreeLogRegVProbMatrix[nodelogi, 1] - DecTreeLogRegVProbMatrix[nodelogi, 3]
}
DecTreeLogRegVPredictions <- 
  DecTreeLogRegVProbMatrix[,1] * (premiership7V$FTR == "H") + 
  DecTreeLogRegVProbMatrix[,2] * (premiership7V$FTR == "D") +
  DecTreeLogRegVProbMatrix[,3] * (premiership7V$FTR == "A")

DecTreeLogRegVClassVector <- matrix2class(DecTreeLogRegVProbMatrix)
cat("Decision Tree + Logistic Regression Validation set likelihood metric",
    likelihood(DecTreeLogRegVPredictions)
)
cat("Decision Tree + Logistic Regression Validation set accuracy",
   accuracy(DecTreeLogRegVClassVector, premiership7V$FTR)
)
...
DecisionTreeClassVector <- matrix2class(DecisionTreeProbMatrix)
...
cat("Decision Tree (alone) K-fold accuracy",
    accuracy(DecisionTreeClassVector, premiership7$FTR)
)
# Validation Set

modelv <- rpart(FTR ~ GDrdiff + GDrdiffLS + GDrdiffLS2, premiership7, method = "class")
DecisionTreeVProbMatrix <- predict(modelv, premiership7V, type = "prob")
colnames(DecisionTreeVProbMatrix) <- c("H", "D", "A")
DecisionTreeVClassVector <- predict(modelv, premiership7V, type = "class")
DecisionTreeVPredictions <- 
  DecisionTreeVProbMatrix[,1] * (premiership7V$FTR == "H") + 
  DecisionTreeVProbMatrix[,2] * (premiership7V$FTR == "D") +
  DecisionTreeVProbMatrix[,3] * (premiership7V$FTR == "A")

cat("Decision Tree (alone) Validation Set likelihood metric",
    likelihood(DecisionTreeVPredictions)
)
cat("Decision Tree (alone) Validation Set accuracy",
    accuracy(DecisionTreeVClassVector, premiership7V$FTR)
)
...
RandomForestClassVector <- matrix2class(RandomForestProbMatrix)
...
cat("Random Forest K-fold accuracy metric",
    accuracy(RandomForestClassVector, premiership7$FTR)
)
# Random Forest
# Validation Set
# note: takes time to process

modelv <- 
  randomForest(FTR ~ GDrdiff + GDrdiffLS + GDrdiffLS2, 
               premiership7, ntree = 2500,
               method = "regression")
RandomForestVProbMatrix <- predict(modelv, premiership7V, type = "prob")
RandomForestVClassVector <- predict(modelv, premiership7V, type = "class")
colnames(RandomForestVProbMatrix) <- c("H", "D", "A")
RandomForestVPredictions <- 
  RandomForestVProbMatrix[,1] * (premiership7V$FTR == "H") + 
  RandomForestVProbMatrix[,2] * (premiership7V$FTR == "D") +
  RandomForestVProbMatrix[,3] * (premiership7V$FTR == "A")

cat("Random Forest Validation Set likelihood metric",
    likelihood(RandomForestVPredictions)
)
cat("Random Forest Validation Set accuracy",
    accuracy(RandomForestVClassVector, premiership7V$FTR)
)
...
LogiRegClassVector <- matrix2class(LogiRegProbMatrix)
LogiRegPredictions <- 
...
cat("Logistic Regression K-fold accuracy",
    accuracy(LogiRegClassVector, premiership7$FTR)
)
# Logistic Regression
# Validation Set
...
LogiRegVClassVector <- matrix2class(LogiRegVProbMatrix)

cat("Logistic Regression Validation Set likelihood metric",
    likelihood(LogiRegVPredictions)
) 
cat("Logistic Regression Validation Set K-fold accuracy",
    accuracy(LogiRegVClassVector, premiership7V$FTR)
)
# NEURAL NETWORK
# k-fold validation 
cat("Neural Network")
...
NeuralNetClassVector <- matrix2class(NeuralNetProbMatrix)
...
cat("NeuralNetwork K-fold accuracy",
    accuracy(premiership7$FTR, NeuralNetClassVector)
)
# Neural Network
# Validation Set
...
NeuralNetVClassVector <- matrix2class(NeuralNetVProbMatrix)
...
cat("Neural Network Class Accuracy",
    accuracy(premiership7V$FTR, NeuralNetVClassVector)
)


# combine results
models <- c("DT + LR", "Decision Tree", "Random Forest", "Logistic Regression", "Neural Network")
k_fold_likelihoods <- c(likelihood(DecTreeLogRegPredictions),
                        likelihood(DecisionTreePredictions),
                        likelihood(RandomForestPredictions), 
                        likelihood(LogiRegPredictions),
                        likelihood(NeuralNetPredictions)
...
k_fold_accuracy <- c(accuracy(DecTreeLogRegClassVector, premiership7$FTR),
                     accuracy(DecisionTreeClassVector, premiership7$FTR),
                     accuracy(RandomForestClassVector, premiership7$FTR),
                     accuracy(LogiRegClassVector, premiership7$FTR),
                     accuracy(NeuralNetClassVector, premiership7$FTR)
                      )

validation_likelihoods <- c(likelihood(DecTreeLogRegVPredictions),
                            likelihood(DecisionTreeVPredictions),
                            likelihood(RandomForestVPredictions), 
                            likelihood(LogiRegVPredictions),
                            likelihood(NeuralNetVPredictions)
...
validation_accuracy <- c(accuracy(DecTreeLogRegVClassVector, premiership7V$FTR),
                        accuracy(DecisionTreeVClassVector, premiership7V$FTR),
                        accuracy(RandomForestVClassVector, premiership7V$FTR),
                        accuracy(LogiRegVClassVector, premiership7V$FTR),
                        accuracy(NeuralNetVClassVector, premiership7V$FTR)
                        )

summary <- data.frame(models, k_fold_likelihoods, k_fold_accuracy, 
